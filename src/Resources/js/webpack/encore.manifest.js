...
const DEFAULT_LOADER_CONFIG_PATH = path.resolve(process.cwd(), '.encore', 'loader.config.json');
...
function configureEncoreBase(options = {}) {
  ...
  if (!loaderConfig.ts.configFile && fs.existsSync(DEFAULT_LOADER_CONFIG_PATH)) {
    try {
      const loaderRuntimeConfig = JSON.parse(fs.readFileSync(DEFAULT_LOADER_CONFIG_PATH, 'utf8'));
      ...
      // Ignore invalid loader config file and keep default tsconfig resolution.
    }
  }
